...
"SELECT Users.name, SUM(Pledges.amount) as total_pledged FROM users JOIN pledges ON Users.id = Pledges.user_id GROUP BY Users.name ORDER BY total_pledged ASC"
end

def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  "SELECT Projects.category, SUM(Pledges.amount) FROM projects JOIN pledges ON Projects.id = Pledges.project_id WHERE Projects.category = GROUP BY Projects.category ORDER BY total_pledged ASC"
end
